...
class LanguageManager: ObservableObject {
    ...
    private init() {
        ...
        // 既存ユーザー向けに、sourceLanguageCodeがシステム言語（例："ja"）の場合はクリア
        // これにより、次回起動時にはデフォルトの"auto"が使用される
        if let savedLanguage = UserDefaults.standard.string(forKey: "sourceLanguageCode") {
            if savedLanguage == "ja" || savedLanguage == "en" || savedLanguage == "zh" {
                print("🔄 Clearing old sourceLanguageCode: \(savedLanguage) -> auto")
